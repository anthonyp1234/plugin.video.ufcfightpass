...
url247 = "https://www.ufc.tv/channel/fight-pass-24-7"
...
def post_auth(creds):
    """Take in Credentials, 
    Return True if passed login check. False otherwise  """
...
def publish_point(video):
    ...
    get a response that ioncludes a path,returns (status code, path to video)"""
    ...
    ###Adding if video["id"] == 1 then change type tt channel
    if video["id"] == "1":
        payload = {
            'id': video['id'],
            'type': 'channel',  ##Type needs to be channel if it is for Streaming 27-7
            'nt': '1',
            'format': 'json'
        }    
        #dialog = xbmcgui.Dialog()
        #result = dialog.yesno('Channel?', 'channel')
...
def get_categories():
    """Using the website base+ fightpass, scans through the data['subcategories'] (gained from get_data(website)), appends title, url, 
    returns dict of paging, and items = list of dict{tite, url, level==top}
    """


    try:
        reader = pickle.load(open(SWAP_FILE, 'rb'))
        #dialog = xbmcgui.Dialog()
        #dialog.ok('Load swap', 'Swap loaded')
    ...
    # Fetch the main UFC Fight Pass category menu data
        ...
        pickle.dump(my_return, open(SWAP_FILE, 'wb'))

        #dialog = xbmcgui.Dialog()
        #dialog.ok('Save swap', 'Swap Saved')
...
def main():
    """Grabs creds, checks to make sure creds check out ok. If so, start get_categories() and building_menu()"""
...
def build_menu(itemData):
    ...
    ### Add 24-7 List item Anthony PINO
    if is_top_level:
        data247 = get_data(url247)
        title = "247Sstream"
        #thumb = data247['thumb']
        item = xbmcgui.ListItem(label=title)     
        url = '{0}?action=play&i={1}&t={2}'.format(addon_url, data247["id"], title)
...
def play_video(v_id, v_title):
    ...
    #dialog = xbmcgui.Dialog()
    #dialog.ok('Stream', 'ID is ' + str(v_id))
    if status == 400:
...
####Anthony Extra functions
def favourites_get():
    q_data = get_pers('https://apis.neulion.com/personalization_ufc/v1/favorite/list')
    if 'contents' in q_data:
        q_ids = [q['id'] for q in q_data['contents']]
